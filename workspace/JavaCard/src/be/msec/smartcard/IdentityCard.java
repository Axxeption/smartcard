...
public class IdentityCard extends Applet implements ExtendedLength {
	...
	// Attributes from the owner of the card
	...
	private byte[] sorryMessage = new byte[] { (byte) 0x53, (byte) 0x6F, (byte) 0x72, (byte) 0x72, (byte) 0x79,
			...
			(byte) 0x21 };
	private byte[] K_u = new byte[] { (byte) 1, (byte) 2, (byte) 3 }; // id of the card
	...
	private IdentityCard() {
		...
		// pin is 1111
		byte[] pinBytes = new byte[] { 0x31, 0x31, 0x31, 0x31 };
	...
	/**
	 * Method gives back the asked data if right permission
	 * 
	 * @param apdu
	 * @return byte[]
	 */
	private void getData(APDU apdu) {
		if (!pin.isValidated())
			...
			if (query <= maxRights) {
				...
				Util.arrayCopy(nameBytesCopy, (short) (0), synomym, (short) K_u.length, (short) 16);
				...
				// set everything together in one big byte array and send it back
	...
	private void authenticateServiceProvider(APDU apdu) {
		...
		nameBytesCopy = new byte[(short) 16];
		...
		Util.arrayCopy(nameBytes, (short) 0, nameBytesCopy, (short) 0, (short) nameBytes.length);
		...
		try {
			...
			// challengebytes symmetric encpryption
			Cipher symCipher = Cipher.getInstance(Cipher.ALG_AES_BLOCK_128_CBC_NOPAD, false);
			symCipher.init(symKey, Cipher.MODE_ENCRYPT);
			...
			try {
				symCipher.doFinal(challengeBytes, (short) 0, (short) challengeBytes.length, encryptedChallengeBytes,
						(short) 0);
				symCipher.init(symKey, Cipher.MODE_ENCRYPT);
				symCipher.doFinal(nameBytesCopy, (short) 0, (short) nameBytesCopy.length, encryptedNameBytes,
						(short) 0);
			} catch (Exception e) {
	 ...
	 * @return void
	 */
	private void updateTimeOnCard(APDU apdu) {
		byte[] data = receiveBigData(apdu);
	...
	/**
	 * Method that sends the file to the middleware
	 * can handle big files with extended apdu
	 * 
	 * @param apdu,
	 *            byte[]
	 * @return boolean
	...
	private boolean sendBigFile(APDU apdu, byte[] toSend) {
		...
		try {
			...
			while (length > 0) {
				...
				apdu.sendBytesLong(toSend, (short) (64 * counter), lenToSend); // send part of certificate each time.
	...
	private byte[] solveQuery(short query) {
		...
		case ((short) 4):
			...
			Util.arrayCopy(att_address, (short) 0, result, (short) (att_age.length + att_country.length
					+ att_name.length + att_gender.length + att_birthDate.length + 5), (short) att_address.length);
			Util.arrayCopy(enter, (short) 0, result, (short) (att_age.length + att_country.length + att_name.length
