...
//			checkChallenge();
	...
	public void checkChallenge() throws Exception {
		...
		if (r.getSW()==SW_VERIFICATION_FAILED) throw new Exception("ERROR");
			...
			byteCertificate = Arrays.copyOfRange( byteCertificate, 6, byteCertificate.length -1);
		...
		byte [] byteCertificate2 = Arrays.copyOfRange( r.getData(), 6, r.getData().length);
		...
		boolean ok = signature.verify(r.getBytes());
